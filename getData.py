...
def save_fiber_toXYZ(file_path,nb_snapshots,skipSnap,setup_number):
    if not os.path.isdir(f'setup{setup_number}/xyz_files'):
        os.mkdir(f'setup{setup_number}/xyz_files')
    data = loadData(file_path, nb_snapshots,skipSnap)
    pos = data[:, :, 3:6]
    ori = data[:, :, 6:9]
    xyzName = os.path.split(file_path)[-1][:-3]+'xyz'
    myfileXY = open(f'setup{setup_number}/xyz_files/'+xyzName, "w")
