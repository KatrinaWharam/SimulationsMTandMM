...
params = {
    ...
    'l1': '4',
    'l2': '3',
    'n2': '-0.07071 0.07071 0'
}

def get_params(dx):
    l1 = float(params['l1'])
    l2 = float(params['l2'])
    lx = np.around(l1 + np.cos(3*np.pi/4)*l2,3)
    ly = np.around(np.sin(3*np.pi/4)*l2,3)
    dlx = np.arange(-lx/2,lx/2+0.001,dx)
    dly = np.arange(-ly/2,ly/2+0.001,dx)
    parameterList = []
    for xpos in dlx:
        for ypos in dly:
